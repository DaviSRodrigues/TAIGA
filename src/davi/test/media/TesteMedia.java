...
public class TesteMedia {
	private static final SUT programa = SUT.Media;
...
//		testeOperadoresGeneticos();
		...
		System.out.println("SUT: Media.java\n");
...
//		
//		double mutationScore = new ImageEvaluator(programa).getFitness(oracle.clone(), null);
//		
...
//		System.out.println("\n" + String.join("", Collections.nCopies(120, "=")) + "\n\n");
	...
	private static void prepararFontesGeradosPeloMuJava() throws Exception {
		File directory = new File("Mutants\\Media");
		...
		for(File f : folders) {
			...
			for(File m : mutants) {
				File destino = new File("src\\davi\\mutation\\media\\" + f.getName() + "\\Media.java");
				...
				i.next();
				bw.write("package davi.mutation.media." + f.getName() + ";\n");
...
//			method.invoke(testCases);
	...
	private static void apagarMutantesEquivalentes() {
		String[] mutantes = "AOIS_100,AOIS_101,AOIS_102,AOIS_103,AOIS_104,AOIS_105,AOIS_106,AOIS_111,AOIS_112,AOIS_87,AOIS_88,AOIS_91,AOIS_92,AOIS_93,AOIS_94,AOIS_95,AOIS_96,AOIS_97,AOIS_98,AOIS_99,SDL_17,SDL_3".split(",");
		
		int deletados = 0;
		
		for(String mutante : mutantes) {
			File pasta = new File("Mutants\\Media\\" + mutante);
...
//				date.get(Calendar.MINUTE) + date.get(Calendar.SECOND) + ".png");
		...
		while(i.hasNext()) {
			...
			candidate.getCandidate().save("Resultados\\Media\\" + pasta + "\\" + j + ".png");
		}
