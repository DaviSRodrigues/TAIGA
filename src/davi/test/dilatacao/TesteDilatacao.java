...
public class TesteDilatacao {
	private static final SUT programa = SUT.Dilatacao;
...
//		testeOperadoresGeneticos();
		...
		System.out.println("SUT: Dilatacao.java\n");
...
//		mostraMutantesVivos();
		...
		//Uma única execução do programa
//		Image oracle = new Image("Geometricas\\Quadrado.png");
//		
//		double mutationScore = new ImageEvaluator(SUT.Mediana).getFitness(oracle.clone(), null);
//		
...
//		System.out.println("\n" + String.join("", Collections.nCopies(120, "=")) + "\n\n");
	...
	private static void prepararFontesGeradosPeloMuJava() throws Exception {
		File directory = new File("Mutants\\Dilatacao");
		...
		for(File f : folders) {
			...
			for(File m : mutants) {
				File destino = new File("src\\davi\\mutation\\dilatacao\\" + f.getName() + "\\Dilatacao.java");
				...
				i.next();
				bw.write("package davi.mutation.dilatacao." + f.getName() + ";\n");
...
//			method.invoke(testCases);
	...
	private static void apagarMutantesEquivalentes() {
		String[] mutantes = "AOIS_100,AOIS_101,AOIS_102,AOIS_111,AOIS_112,AOIS_85,AOIS_86,AOIS_87,AOIS_88,AOIS_89,AOIS_90,AOIS_91,AOIS_92,AOIS_93,AOIS_94,AOIS_95,AOIS_96,AOIS_97,AOIS_98,AOIS_99,SDL_3,SDL_18".split(",");
		
		int deletados = 0;
		
		for(String mutante : mutantes) {
			File pasta = new File("Mutants\\Dilatacao\\" + mutante);
...
//				date.get(Calendar.MINUTE) + date.get(Calendar.SECOND) + ".png");
		...
		while(i.hasNext()) {
			...
			candidate.getCandidate().save("Resultados\\Dilatacao\\" + pasta + "\\" + j + ".png");
		}
