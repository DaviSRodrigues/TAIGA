...
public class TesteMediana {
	private static final SUT programa = SUT.Mediana;
...
//		testeOperadoresGeneticos();
		...
		System.out.println("SUT: Mediana.java\n");
...
//		System.out.println("\n" + String.join("", Collections.nCopies(120, "=")) + "\n\n");
	...
	private static void prepararFontesGeradosPeloMuJava() throws Exception {
		File directory = new File("Mutants\\Mediana");
		...
		for(File f : folders) {
			...
			for(File m : mutants) {
				File destino = new File("src\\davi\\mutation\\mediana\\" + f.getName() + "\\Mediana.java");
				...
				i.next();
				bw.write("package davi.mutation.mediana." + f.getName() + ";\n");
...
//			method.invoke(testCases);
	...
	private static void apagarMutantesEquivalentes() {
		...
		for(String mutante : mutantes) {
			File pasta = new File("Mutants\\Mediana\\" + mutante);
...
//				date.get(Calendar.MINUTE) + date.get(Calendar.SECOND) + ".png");
		...
		while(i.hasNext()) {
			...
			candidate.getCandidate().save("Resultados\\Mediana\\" + pasta + "\\" + j + ".png");
		}
