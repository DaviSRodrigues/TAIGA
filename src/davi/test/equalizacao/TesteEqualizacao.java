...
public class TesteEqualizacao {
	private static final SUT programa = SUT.Equalizacao;
...
//		testeOperadoresGeneticos();
		...
		System.out.println("SUT: Equalizacao.java\n");
...
//		System.out.println("\n" + String.join("", Collections.nCopies(120, "=")) + "\n\n");
	...
	private static void prepararFontesGeradosPeloMuJava() throws Exception {
		File directory = new File("Mutants\\Equalizacao");
		...
		for(File f : folders) {
			...
			for(File m : mutants) {
				File destino = new File("src\\davi\\mutation\\equalizacao\\" + f.getName() + "\\Equalizacao.java");
				...
				i.next();
				bw.write("package davi.mutation.equalizacao." + f.getName() + ";\n");
...
//			method.invoke(testCases);
	...
	private static void apagarMutantesEquivalentes() {
		String[] mutantes = "AOIS_41,AOIS_42,AOIS_55,AOIS_56,AOIS_57,AOIS_58,ROR_15,ROR_19,ROR_20,ROR_26,ROR_5,SDL_12,SDL_16,SDL_6".split(",");
		
		int deletados = 0;
		
		for(String mutante : mutantes) {
			File pasta = new File("Mutants\\Equalizacao\\" + mutante);
...
//				date.get(Calendar.MINUTE) + date.get(Calendar.SECOND) + ".png");
		...
		while(i.hasNext()) {
			...
			candidate.getCandidate().save("Resultados\\Equalizacao\\" + pasta + "\\" + j + ".png");
		}
