...
public class TesteLimpeza {
	private static final SUT programa = SUT.Limpeza;
...
//		testeOperadoresGeneticos();
		...
		System.out.println("SUT: Limpeza.java\n");
...
//		System.out.println("\n" + String.join("", Collections.nCopies(120, "=")) + "\n\n");
	...
	private static void prepararFontesGeradosPeloMuJava() throws Exception {
		File directory = new File("Mutants\\Limpeza");
		...
		for(File f : folders) {
			...
			for(File m : mutants) {
				File destino = new File("src\\davi\\mutation\\limpeza\\" + f.getName() + "\\Limpeza.java");
				...
				i.next();
				bw.write("package davi.mutation.limpeza." + f.getName() + ";\n");
...
//			method.invoke(testCases);
	...
	private static void apagarMutantesEquivalentes() {
		String[] mutantes = "AOIS_121,AOIS_122,AOIS_123,AOIS_124,AOIU_18,AORB_4,ODL_8,ROR_12,ROR_19,ROR_26,ROR_29,ROR_39,ROR_46,ROR_5,ROR_54,ROR_61,SDL_12,SDL_16,SDL_21,SDL_24,SDL_32,SDL_37,SDL_7,VDL_8".split(",");
		
		int deletados = 0;
		
		for(String mutante : mutantes) {
			File pasta = new File("Mutants\\Limpeza\\" + mutante);
...
//				date.get(Calendar.MINUTE) + date.get(Calendar.SECOND) + ".png");
		...
		while(i.hasNext()) {
			...
			candidate.getCandidate().save("Resultados\\Limpeza\\" + pasta + "\\" + j + ".png");
		}
